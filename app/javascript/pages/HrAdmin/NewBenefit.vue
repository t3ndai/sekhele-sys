<template>
    <div>
        <div>
            <Button
                label="New Benefit"
                icon="pi pi-plus"
                @click="visible = true"
                class="benefit-btn"
            />
        </div>
        <Dialog
            v-model="visible"
            header="New Benefit"
            :visible="visible"
            @update:visible="visible = $event"
        >
            <form @submit.prevent="saveBenefit">
                <div class="flex flex-col mb-2 field">
                    <label for="name">Name</label>
                    <InputText id="name" v-model="form.name" />
                </div>

                <div class="field mb-2">
                    <Select
                        v-model="form.benefit_type_id"
                        :options="benefit_types"
                        optionLabel="name"
                        optionValue="id"
                        placeholder="Select Benefit Type"
                    />
                </div>

                <div class="field mb-2">
                    <fieldset class="radios">
                        <legend>Valuation Type</legend>
                        <div class="flex gap-x-2">
                            <div class="flex">
                                <RadioButton
                                    v-model="form.valuation_type"
                                    inputId="valuation_type"
                                    name="valuation_type"
                                    value="taxable"
                                />
                                <label for="status">Taxable </label>
                            </div>
                            <div class="flex">
                                <RadioButton
                                    v-model="form.valuation_type"
                                    inputId="valuation_type"
                                    name="valuation_type"
                                    value="non_taxable"
                                />
                                <label for="status">Non-Taxable </label>
                            </div>
                        </div>
                    </fieldset>
                </div>

                <div class="mt-2">
                    <Button
                        class="benefit-btn"
                        label="Save"
                        icon="pi pi-save"
                        type="submit"
                        :disabled="form.processing"
                    />
                </div>
            </form>
        </Dialog>
    </div>
</template>

<script setup>
import { Dialog, Button, Select, RadioButton, InputText } from "primevue";
import { ref, computed } from "vue";
import { useForm, usePage } from "@inertiajs/vue3";

const page = usePage();
const org_id = computed(() => page.props.org_id).value;

defineProps({
    benefit_types: Array,
});

const form = useForm({
    name: "",
    valuation_type: "",
    benefit_type_id: "",
});

const visible = ref(false);

function saveBenefit() {
    form.post(`/organizations/${org_id}/benefits`, {
        onSuccess: () => {
            form.reset();
            visible.value = false;
        },
    });
}
</script>

<style scoped>
.benefit-btn {
    background-color: orangered;
    border: none;

    :hover {
        background-color: #ffb59a;
    }
}
</style>
